...
local function write(text, object, secondLabel)
    if object:IsA("TextLabel") then
        ...
        -- Split the text into lines using single '\n'
        for i = 1, #text do
        ...
        for lineIndex, line in ipairs(lines) do
            ...
            -- Add two new lines after each line is fully written
            if lineIndex < #lines then
                object.Text = object.Text .. "\n\n"
            end

            -- After the 5th line, start writing the second label below the last one
            if lineIndex == 5 and secondLabel then
                secondLabel.Text = ""  -- Clear the second label's text
                secondLabel.Visible = true  -- Make the second label visible
                secondLabel.Position = UDim2.new(0.35, 0, 0.1, 0)  -- Position just below "Z TO CYCLE TYPE"
                write("CURRENT TYPE || NORMAL", secondLabel)  -- Start typing for the second label
            end
        end
    end
end
...
-- Second label (initially hidden, will appear below the last created label)
local secondLabel = Instance.new("TextLabel")
secondLabel.Parent = starter
secondLabel.BackgroundTransparency = 1
secondLabel.BorderSizePixel = 0
secondLabel.Font = Enum.Font.SciFi
secondLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
secondLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
secondLabel.TextStrokeTransparency = 1
secondLabel.TextWrapped = true
secondLabel.TextSize = 16
secondLabel.Visible = false  -- Initially hidden
secondLabel.Text = ""
secondLabel.Size = UDim2.new(0.3, 0, 0.05, 0)

-- Third label for find
...
find.Position = UDim2.new(0.35,0,0.2,0)
...
-- Setup the color cycling for the labels (including secondLabel and find)
delay(0, function()
    while wait() do
        for change = 1, 255 do
            ...
            secondLabel.TextColor3 = Color3.fromHSV(change / 255, 1, 1)  -- Rainbow effect for "hi"
            wait()
        end
    end
end)
...
delay(0.1, function()
    local lines = {"PRESS Q TO SUMMON A DOGE", "Z TO CYCLE TYPE", "CURRENT STATUS", "MOVE TO", "CLICK TO JUMP", "hi"}
    local fullText = table.concat(lines, "\n")  -- Use \n to separate lines
    write(fullText, label, secondLabel)  -- Pass the second label to start after the 5th line
end)
